 ...
 * Prevents re-renders of the entire list when one item is selected.
 */
const CandidateItem = memo(({ candidate, isSelected, onToggle }) => {
  return (
    <div 
      onClick={() => onToggle(candidate.id)}
      className={`relative p-4 rounded-[12px] border-[1.5px] transition-all cursor-pointer hover:shadow-md ${
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              {candidate.verified && <Star size={14} className="text-[#F5831F] fill-[#F5831F]" aria-label="Verified Candidate" />}
            </div>
            <p className="text-sm font-bold text-[#F5831F] mt-0.5">{candidate.party}</p>
          </div>
          <div className="w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center border border-gray-200 flex-shrink-0 text-xl" aria-hidden="true">👤</div>
        </div>

        <div className="grid grid-cols-1 gap-y-2 mt-3 pt-3 border-t border-gray-100">
          <div className="flex items-center gap-2">
            <GraduationCap size={14} className="text-gray-400" />
            <span className="text-xs text-gray-600 font-medium truncate">{candidate.education}</span>
          </div>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Scale size={14} className={candidate.criminal_cases > 0 ? 'text-red-500' : 'text-green-500'} />
              <span className={`text-xs font-bold ${candidate.criminal_cases > 0 ? 'text-red-600' : 'text-green-600'}`}>
                {candidate.criminal_cases} Criminal Cases
              </span>
            </div>
            <div className="flex items-center gap-2">
              <BadgeIndianRupee size={14} className="text-gray-400" />
              <span className="text-xs font-bold text-gray-800">{candidate.total_assets} Assets</span>
            </div>
          </div>
        </div>
      </div>
...
CandidateItem.propTypes = {
  candidate: PropTypes.shape({
    ...
    criminal_cases: PropTypes.number,
...
const Candidates = () => {
  ...
  // useMemo for filtering to improve efficiency
  ...
  // useMemo for selected candidates list
  const selectedCandidates = useMemo(() => {
    return candidates.filter(c => selectedIds.includes(c.id));
  }, [candidates, selectedIds]);

  return (
    <div className="h-full flex flex-col relative bg-[#F8F9FA]" role="main" aria-label="Candidate Search and Comparison">
      {/* Header Area */}
      ...
      <div className="flex-1 overflow-y-auto px-5 pt-4 pb-[140px]" role="list">
        {isLoading ? (
          ...
          <div className="space-y-3">
            ...
            {filtered.length === 0 && (
              <p className="text-center text-gray-400 py-8">No candidates found matching your search.</p>
            )}
          </div>
        )}
      </div>
      ...
      {selectedIds.length >= 2 && (
        <div className="fixed bottom-[65px] left-1/2 -translate-x-1/2 z-[100] w-[calc(100%-32px)] max-w-[398px]">
          <button 
            ...
            aria-label={`Compare ${selectedIds.length} selected candidates`}
          >
            <GitCompare size={20} />
      ...
      {/* Comparison Modal (Skeleton implemented as per UI design) */}
      {showComparison && (
        <>
          <div className="fixed inset-0 bg-black/50 z-[199] animate-fade-in" onClick={() => setShowComparison(false)} />
          <div className="fixed top-[60px] bottom-[65px] left-1/2 -translate-x-1/2 w-full max-w-[430px] bg-white rounded-t-[20px] z-[200] flex flex-col shadow-2xl animate-slide-up" role="dialog" aria-modal="true">
            <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white rounded-t-[20px] z-10">
              ...
              <button onClick={() => setShowComparison(false)} className="w-10 h-10 rounded-full bg-gray-100 flex items-center justify-center text-gray-500 hover:bg-gray-200 transition-colors">
                <X size={20} />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto p-6">
               {/* Comparison content as viewed previously */}
                  ...
                  <table className="w-full border-collapse min-w-[320px]">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      <tr>
                        ...
                        {selectedCandidates.map(c => <td key={c.id} className="py-4 text-center text-xs font-semibold text-gray-800 px-2">{c.education}</td>)}
                      </tr>
                      <tr>
                        <td className="py-4 text-[10px] font-bold text-gray-400 uppercase tracking-wider">Criminal Cases</td>
                        {selectedCandidates.map(c => <td key={c.id} className="py-4 text-center text-xs font-bold text-red-600">{c.criminal_cases}</td>)}
                      </tr>
                    </tbody>
                  </table>
               </div>
